# 實作進度看板

> 使用方式：每次任務狀態改變就更新本檔。
> 狀態欄位限定：`Todo` / `In Progress` / `Done` / `Blocked`

---

## 一、總體進度

| 里程碑 | 狀態 | 完成度 |
|---|---|---:|
| M0 專案骨架 | Todo | 0% |
| M1 核心迴圈 | Todo | 0% |
| M2 學習迴圈 | Todo | 0% |
| M3 局外迴圈 | Todo | 0% |
| M4 內容平衡 | Todo | 0% |
| Alpha Gate | Todo | 0% |

---

## 二、任務追蹤

| ID | 任務 | 里程碑 | Owner | 狀態 | 備註 |
|---|---|---|---|---|---|
| M0-01 | Unity 專案骨架與目錄 | M0 | TBD | Todo | |
| M0-02 | 五大 V2 Manager stubs | M0 | TBD | Todo | |
| M0-03 | Config 驗證串接 | M0 | TBD | Todo | |
| M0-04 | 最小測試入口 | M0 | TBD | Todo | |
| M1-01 | 牌型判定引擎 | M1 | TBD | Todo | |
| M1-02 | 分數公式與拆解 | M1 | TBD | Todo | |
| M1-03 | 盲注流程 | M1 | TBD | Todo | |
| M1-04 | 商店流程 | M1 | TBD | Todo | |
| M2-01 | Lv0-4 行為模型 | M2 | TBD | Todo | |
| M2-02 | 答錯三選一 | M2 | TBD | Todo | |
| M2-03 | 退化規則 | M2 | TBD | Todo | |
| M2-04 | Boss 學習規則 | M2 | TBD | Todo | |
| M3-01 | XP/LP 結算 | M3 | TBD | Todo | |
| M3-02 | 契約系統 | M3 | TBD | Todo | |
| M3-03 | LP 上限守門 | M3 | TBD | Todo | |
| M3-04 | 課程樹 MVP 串接 | M3 | TBD | Todo | |
| M4-01 | 詞庫內容填充 | M4 | TBD | Todo | |
| M4-02 | 商店權重平衡 | M4 | TBD | Todo | |
| M4-03 | 盲注曲線平衡 | M4 | TBD | Todo | |
| M4-04 | 首輪平衡報告 | M4 | TBD | Todo | |
| A-01 | 全流程回歸 | Alpha | TBD | Todo | |
| A-02 | 存檔/migration 壓測 | Alpha | TBD | Todo | |
| A-03 | 效能與穩定性驗收 | Alpha | TBD | Todo | |
| A-04 | 發版決策 | Alpha | TBD | Todo | |
