...
| W6 | M3 局外迴圈 | XP/LP、契約、課程樹 4x12 | Meta/System |
...
- M3-01 已完成（XP/LP 結算）。
- M3-02 已完成（契約生成/結算）。
- M3-03 已完成（LP 45% 上限守門）。
- M3-04 已完成（課程樹 4x12 串接）。
- M4-01 已完成（T1/T2 詞庫可玩內容）。
- M4-02 已完成（商店池權重與價格帶調整）。
- M4-03 已完成（盲注曲線平衡）。
- M4-04 已完成（首輪平衡報告）。
- A-01 已完成：可授權 Unity 環境已執行 EditMode 回歸，Ante 1-8 與失敗分支測試通過。
- A-UI-01 已完成：可操作卡牌 UI 原型（拖曳上桌、答題流程、商店卡片格、可收合調參）。
- S7 優先驗測已完成：Final Gate（95%/100%+7 天）與 Endless 長局穩定性已補齊證據。
- S8/S9 驗測已補齊：Telemetry 三模型與 NFR 三模型壓測基線已落地。
- 目前主軸：A-02（存檔/migration 壓測）與 A-03（效能穩定性驗收）。
